...
internal static partial class SslStreamPal
{
    ...
    // Helper method to check if async operations are supported for a context
    internal static bool SupportsAsyncOperations(SafeDeleteContext? context)
    {
        return UseAsyncDecrypt && context is SafeDeleteNwContext;
    }

    public static void GetPendingWriteData(SafeDeleteContext context, ref ProtocolToken protocolToken)
    {
        if (NetEventSource.Log.IsEnabled()) NetEventSource.Info(context, "", "GetPendingWriteData");
        ((SafeDeleteNwContext)context).ReadPendingWrites(ref protocolToken);
    }
    ...
    public static Task<SecurityStatusPalErrorCode>? ExtractHandshakeTask(SafeFreeCredentials? _, SafeDeleteContext context)
    {
        var nwContext = (SafeDeleteNwContext)context;
        var bytesReady = nwContext.BytesReadyForConnection;
        if (NetEventSource.Log.IsEnabled()) NetEventSource.Info(context, $"BytesReadyForConnection: {bytesReady}", "ExtractHandshakeTask");
        return bytesReady > 0 ? Task.FromResult(SecurityStatusPalErrorCode.ContinueNeeded) : nwContext.HandshakeTaskSource?.Task;
    }
