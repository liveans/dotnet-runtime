...
using NwOSStatus = Interop.NetworkFramework.OSStatus;
...
namespace System.Net.Security
{
    ...
    /// Network Framework APIs instead of the legacy SecureTransport APIs.
    /// </summary>
    internal sealed class SafeDeleteNwContext : SafeDeleteContext
    {
        ...
        private ManualResetEventSlim _readWaiter = new();
        private ManualResetEventSlim _writeWaiter = new();

        // TaskSourceCompletion objects
        private TaskCompletionSource<SecurityStatusPalErrorCode>? _handshakeTaskSource;
        internal TaskCompletionSource<SecurityStatusPalErrorCode>? DecryptTask { get; private set; }

        internal TaskCompletionSource<SecurityStatusPalErrorCode>? HandshakeTaskSource
        {
            get
            {
                return _handshakeTaskSource;
            }
        }
        ...
        public SafeDeleteNwContext(SslAuthenticationOptions sslAuthenticationOptions) : base(IntPtr.Zero)
        {
            ...
            _handshakeTaskSource = new TaskCompletionSource<SecurityStatusPalErrorCode>(TaskCreationOptions.RunContinuationsAsynchronously);
        }
        ...
        private static bool CheckNetworkFrameworkAvailability()
        {
            try
            {
                unsafe
                {
                    ...
                    return !Interop.NetworkFramework.Tls.Init(&StatusUpdateCallback, &WriteToConnection);
                }
            }
            catch
            {
...
#pragma warning restore SYSLIB0039
        ...
        private static unsafe int WriteToConnection(IntPtr thisHandle, byte* data, void** dataLength)
        {
            ...
            {
                return (int)NwOSStatus.WriteError;
            }
        ...
        private void WriteToConnection(ReadOnlySpan<byte> data)
        {
            ...
            lock (_lockObject)
            {
                ...
                if (!_handshakeDone)
                {
                    if (NetEventSource.Log.IsEnabled()) NetEventSource.Info(this, "Handshake in progress, resetting handshake task source", "WriteToConnection");
                    var handshakeTaskSource = _handshakeTaskSource;
                    _handshakeTaskSource = new TaskCompletionSource<SecurityStatusPalErrorCode>(TaskCreationOptions.RunContinuationsAsynchronously);
                    handshakeTaskSource?.TrySetResult(SecurityStatusPalErrorCode.ContinuePending);
                    if (NetEventSource.Log.IsEnabled()) NetEventSource.Info(this, "Handshake task source reset and set to ContinuePending", "WriteToConnection");
                }
            }
        ...
        internal int BytesReadyFromConnection
        {
            get
            {
                lock (_lockObject)
                {
                    ...
                    return _readStatus == (int)NwOSStatus.NoError ? 0 : -1;
                }
            }
        }
        ...
        {
            if (NetEventSource.Log.IsEnabled()) NetEventSource.Info(this, $"Starting - disposed: {_disposed}, handshakeStarted: {_handshakeStarted}, handshakeDone: {_handshakeDone}", "PerformHandshake");
            ObjectDisposedException.ThrowIf(_disposed, this);

            ReadOnlySpan<byte> inputBuffer = _inputBuffer.ActiveSpan;
            if (NetEventSource.Log.IsEnabled()) NetEventSource.Info(this, $"Input buffer length: {inputBuffer.Length}", "PerformHandshake");

            if (_handshakeStarted && inputBuffer.Length == 0)
            {
                if (NetEventSource.Log.IsEnabled()) NetEventSource.Info(this, "Handshake started but no input data - returning OK", "PerformHandshake");
                return new SecurityStatusPal(SecurityStatusPalErrorCode.OK);
            }
            else
            {
                if (NetEventSource.Log.IsEnabled()) NetEventSource.Info(this, $"Either handshake is not started or input buffer is not empty here are values: handshakeStarted: {_handshakeStarted}, inputBuffer.Length: {inputBuffer.Length}", "PerformHandshake");
            }

            if (_handshakeDone)
            {
                if (NetEventSource.Log.IsEnabled()) NetEventSource.Info(this, "Handshake already done - returning ContinueNeeded", "PerformHandshake");
                return new SecurityStatusPal(SecurityStatusPalErrorCode.ContinueNeeded);
            }

            if (_framerHandle != null && inputBuffer.Length > 0)
            {
                if (NetEventSource.Log.IsEnabled()) NetEventSource.Info(this, "Processing input data with framer", "PerformHandshake");
                ObjectDisposedException.ThrowIf(_disposed, this);
                ...
                if (NetEventSource.Log.IsEnabled()) NetEventSource.Info(this, "Input data processed - returning ContinuePending", "PerformHandshake");
                return new SecurityStatusPal(SecurityStatusPalErrorCode.ContinuePending);
            }

            if (inputBuffer.Length == 0)
            {
                if (NetEventSource.Log.IsEnabled()) NetEventSource.Info(this, "Starting TLS handshake", "PerformHandshake");
                Debug.Assert(_handshakeStarted == false);
                ...
                if (NetEventSource.Log.IsEnabled()) NetEventSource.Info(this, "About to call StartTlsHandshake", "PerformHandshake");
                try
                {
                    Interop.NetworkFramework.Tls.StartTlsHandshake(_connectionHandle, GCHandle.ToIntPtr(_thisHandle));
                    if (NetEventSource.Log.IsEnabled()) NetEventSource.Info(this, "StartTlsHandshake call completed successfully", "PerformHandshake");
                }
                catch (Exception ex)
                {
                    if (NetEventSource.Log.IsEnabled()) NetEventSource.Error(this, $"StartTlsHandshake failed with exception: {ex}", "PerformHandshake");
                    throw;
                }
            }

            if (NetEventSource.Log.IsEnabled()) NetEventSource.Info(this, "Returning ContinuePending", "PerformHandshake");
            return new SecurityStatusPal(SecurityStatusPalErrorCode.ContinuePending);
        }
        ...
        private void FramerStartCallback(SafeNwHandle framerHandle)
        {
            ...
            if (_handshakeStarted && !_handshakeDone)
            {
        ...
        private void HandshakeFinished()
        {
            ...
            _handshakeTaskSource?.TrySetResult(SecurityStatusPalErrorCode.OK);
            _handshakeTaskSource = null;
            _handshakeDone = true;
        }
        ...
        private void HandshakeFailed(int errorCode)
        {
            ...
            _handshakeTaskSource?.TrySetException(ex);
            _handshakeTaskSource = null;
            _handshakeDone = true;
        ...
        private void ConnectionCancelled()
        {
            ...
            HandshakeTaskSource?.TrySetException(ex);
            _handshakeTaskSource = null;
